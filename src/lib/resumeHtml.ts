...
export function buildResumeHtml(data: ResumeData, options?: { compact?: boolean }): string {
  const templateId = data?.settings?.template || 'classic';
  let body = '';

  switch (templateId) {
    case 'modern':
      body = renderModern(data);
      break;
    case 'minimal':
      body = renderMinimal(data);
      break;
    case 'developer':
      body = renderDeveloper(data);
      break;
    case 'student':
      body = renderStudent(data);
      break;
    case 'classic':
    default:
      body = renderClassic(data);
      break;
  }
